...
const Editor = () => {
  ...
  }, [id, loadDiaryById, navigate, resetCurrentDiary]);
  ...
  const handleRemoveTag = (tagToRemove) => {
    updateCurrentDiary(
      "tags",
      currentDiary.tags.filter((tag) => tag !== tagToRemove)
    );
  };
  ...
  const takePicture = async () => {
    try {
      ...
      const image = await Camera.getPhoto({
        quality: 90,
        allowEditing: true,
        resultType: 'uri', // 使用base64格式，便于存储
      });

      // 将照片添加到当前日记
      if (image && image.webPath) {
        updateCurrentDiary("photos", [...currentDiary.photos, image.webPath]);
      }
    } catch (error) {
  ...
  return (
    <div className="pb-20 pt-4">
      <div className="px-4 mb-4 flex justify-between items-center">
        <h1 className="text-2xl font-bold">{id ? "编辑日记" : "新日记"}</h1>
        <button
